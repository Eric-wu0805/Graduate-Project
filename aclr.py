...
def main():
    ...
    print(f"\nFeature count: {len(numerical_cols) + len(categorical_cols)}")
    ...
    # 應用特徵選擇
    selected_features = select_features(X_numeric_scaled, df['label'])
    ...
    # 數據增強
    X_aug, y_aug = augment_data(X, df['label'].values)
    X = np.vstack([X, X_aug])
    y = np.concatenate([df['label'].values, y_aug])
