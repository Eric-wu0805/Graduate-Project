...
# Prediction function
def _apply_feature_masking(X_batch: torch.Tensor, mask_prob: float, mode: str = 'element', mask_value: float = 0.0) -> torch.Tensor:
    """
    測試/驗證也可選擇套用的特徵遮罩（預設不啟用）。
    - mode='element': 每個元素以 mask_prob 機率遮罩
    - mode='feature': 整個批次共享同一組被遮罩的欄位
    """
    if mask_prob <= 0:
        return X_batch
    if mode == 'feature':
        if X_batch.dim() == 2:
            ...
            feature_mask = (torch.rand(1, num_features, device=X_batch.device) < mask_prob)
            return X_batch.masked_fill(feature_mask, mask_value)
        elif X_batch.dim() == 3:
            _, _, num_features = X_batch.shape
            feature_mask = (torch.rand(1, 1, num_features, device=X_batch.device) < mask_prob)
            return X_batch.masked_fill(feature_mask, mask_value)
        else:
            return X_batch
    else:
...
def predict(model, X, device='cpu', batch_size=256,
            use_feature_masking: bool = False,
            feature_mask_prob: float = 0.0,
            feature_mask_mode: str = 'element',
            feature_mask_value: float = 0.0):
    model.eval()
    predictions = []
    total_samples = len(X)
    with torch.no_grad():
        for i in range(0, total_samples, batch_size):
            ...
            if use_feature_masking and feature_mask_prob > 0:
                X_tensor = _apply_feature_masking(X_tensor, feature_mask_prob, feature_mask_mode, feature_mask_value)
...
def main():
    ...
    #final_meta_model = joblib.load('final_meta_model_rf.pkl') # 使用RF
    ...
    for model, model_type in best_models:
        preds = predict(
            model,
            X_cross_cnn if model_type == "cnn" else X_cross,
            device,
            # 預設關閉測試時的遮罩，避免影響客觀評估；如需壓力測試可改為 True
            use_feature_masking=False,
            feature_mask_prob=0.0,
            feature_mask_mode='feature',
            feature_mask_value=0.0
        )
