...
def main():
    ...
    def xgb_objective(trial):
        ...
        kf = KFold(n_splits=3, shuffle=True, random_state=42)
        scores = []
        for train_idx, val_idx in kf.split(meta_X_train_res):
            X_train, X_val = meta_X_train_res[train_idx], meta_X_train_res[val_idx]
    ...
    print("正在用 Optuna 搜尋 XGBoost 最佳參數...")
    study = optuna.create_study()
    study.optimize(xgb_objective, n_trials=30)
    print("最佳參數：", study.best_params)

    best_params = study.best_params
    best_params['scale_pos_weight'] = (y_res == 0).sum() / (y_res == 1).sum()
    best_params['use_label_encoder'] = False
    best_params['eval_metric'] = 'logloss'
    best_params['random_state'] = 42
    best_params['n_jobs'] = -1

    final_meta_model = XGBClassifier(**best_params)
