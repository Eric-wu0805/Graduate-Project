...
def preprocess_data_aclr_style(df, scalers, label_encoders, selected_features, final_scaler):
    ...
    if 'labels' in df.columns:
        ...
        print(f"[標籤自動偵測] labels 欄位唯一值: {label_values}")
        # 常見二元分類自動判斷
        if set(label_values) <= {0, 1}:
            print("[標籤自動偵測] 偵測到 0/1 二元標籤，直接使用。")
            y = df['labels'].astype(np.float32)
        elif set(label_values) <= {'0', '1'}:
            print("[標籤自動偵測] 偵測到 '0'/'1' 字串標籤，轉為 float。");
            y = df['labels'].astype(float)
        elif any(str(v).lower() == 'normal' for v in label_values):
            print("[標籤自動偵測] 偵測到 'normal'，將 'normal' 設為 0，其餘為 1。")
            y = (df['labels'].astype(str).str.lower() != 'normal').astype(np.float32)
        elif any(str(v).upper() == 'BENIGN' for v in label_values):
            print("[標籤自動偵測] 偵測到 'BENIGN'，將 'BENIGN' 設為 0，其餘為 1。")
            y = (df['labels'].astype(str).str.upper() != 'BENIGN').astype(np.float32)
        elif any(str(v).lower() == 'attack' for v in label_values):
            print("[標籤自動偵測] 偵測到 'attack'，將 'attack' 設為 1，其餘為 0。")
            y = (df['labels'].astype(str).str.lower() == 'attack').astype(np.float32)
        else:
            print("[標籤自動偵測] 未知標籤型態，將第一個值設為0，其餘為1。建議手動檢查！")
            y = (df['labels'] != label_values[0]).astype(np.float32)
        print(f"[標籤自動偵測] 轉換後標籤分布: {pd.Series(y).value_counts().to_dict()}")
    return X, X_cnn, y
...
def main():
    ...
    # 顯示cross-dataset內容
    print("=== cross_df 前幾筆資料 ===")
    print(cross_df.head())
    print("=== cross_df 欄位名稱 ===")
    print(cross_df.columns.tolist())
    if 'labels' in cross_df.columns:
        print("=== cross_df['labels'] 標籤分布 ===")
        print(cross_df['labels'].value_counts())
        print("=== cross_df['labels'] 唯一值 ===")
        print(cross_df['labels'].unique())
    else:
        print("cross_df 沒有 'labels' 欄位，請檢查欄位名稱！")
