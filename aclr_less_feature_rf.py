...
def main():
    ...
    categorical_cols = ['protocol_type', 'flag']  # protocol_type 和 flag 是類別型
    label_col = 'labels'
    ...
    for col in numerical_cols:
        # 這裡不再根據偏態做log，因為前面已經對src_bytes、dst_bytes、duration做過log1p
    ...
    # Step 1: 先做 SMOTE 平衡資料
    smote = SMOTE(random_state=42)
    X_resampled, y_resampled = smote.fit_resample(X, y)

    # Step 2: 再切成 8:2 的訓練/驗證集
    ...
    train_model(final_ann, train_loader_full, val_loader_full, device, epochs=10, model_name="Final_ANN",
    ...
    train_model(final_cnn, train_loader_full, val_loader_full, device, epochs=10, model_name="Final_CNN",
                use_dynamic_weighting=True, weight_update_frequency=3, l1_lambda=0.0, l2_lambda=0.001)
    torch.save(final_cnn.state_dict(), 'final_cnn.pth')

    print("\n--- 訓練最終 RNN 模型 ---")
    final_rnn = RNNModel(X.shape[1]).to(device)
    train_model(final_rnn, train_loader_full, val_loader_full, device, epochs=10, model_name="Final_RNN",
    ...
    train_model(final_lstm, train_loader_full, val_loader_full, device, epochs=10, model_name="Final_LSTM",
                use_dynamic_weighting=True, weight_update_frequency=3, l1_lambda=0.0, l2_lambda=0.005)
    torch.save(final_lstm.state_dict(), 'final_lstm.pth')

    # ========== Meta-model 全資料訓練 ==========
    print("\n--- 訓練 Meta-model (RandomForest) ---")
    ...
    for class_id, count in zip(unique_classes, class_counts):
        class_weights[class_id] = total_samples / (len(unique_classes) * count)

    # 使用RandomForest替代XGBoost
    ...
    print("\nFinal Meta-model (RandomForest) Performance (on all training data):")
    ...
    joblib.dump(final_meta_model, 'final_meta_model_rf.pkl')
