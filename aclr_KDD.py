...
def evaluate_model(model, val_loader, device='cpu', class_weighting=None):
    ...
    macro_f1 = f1_score(all_targets, pred_labels, average='macro', zero_division=0)
    macro_recall = recall_score(all_targets, pred_labels, average='macro', zero_division=0)
    pr_auc = average_precision_score(all_targets, all_predictions, average='macro')
    return val_loss / len(val_loader), macro_f1, macro_recall, pr_auc
...
def main():
    ...
    print("[階段] 建立 DataLoader...")
    train_dataset_full = BotnetDataset(X_train, y_train)
    ...
    print(f"[階段] 設定設備: {device}")

    def train_with_early_stop(model, train_loader, val_loader, device, model_name, max_epochs=30, patience=5):
        """
        patience: 允許指標在多少個 epoch 內沒有改善，如果超過就停止訓練（如 patience=5 則5個epoch沒進步就停下）
        restore_best_weights: 訓練結束後自動恢復到val_loss最好的那次模型參數
        """
        best_f1 = -float('inf')
        best_state = None
        patience_counter = 0
        train_losses, train_accs, val_f1s, val_recalls, val_praucs = [], [], [], [], []
        for epoch in range(max_epochs):
            progress = (epoch + 1) / max_epochs * 100
            print(f"[{model_name}] Epoch {epoch+1}/{max_epochs} 進度: {progress:.1f}% 開始訓練... (patience={patience}, restore_best_weights=True)")
            model.train()
            epoch_loss = 0.0
            correct = 0
            total = 0
            for X_batch, y_batch in tqdm(train_loader, desc=f"{model_name} Epoch {epoch+1}/{max_epochs}", leave=False):
                X_batch, y_batch = X_batch.to(device), y_batch.to(device)
                optimizer = optim.AdamW(model.parameters(), lr=0.001)
                optimizer.zero_grad()
                outputs = model(X_batch)
            ...
            val_loss, macro_f1, macro_recall, pr_auc = evaluate_model(model, val_loader, device)
            val_f1s.append(macro_f1)
            val_recalls.append(macro_recall)
            val_praucs.append(pr_auc)
            print(f"[{model_name}] Epoch {epoch+1}/{max_epochs} - Train Loss: {train_losses[-1]:.4f}, Train Acc: {train_accs[-1]:.4f} | Val macro-F1: {macro_f1:.4f}, macro-Recall: {macro_recall:.4f}, PR-AUC: {pr_auc:.4f}")
            print(f"[{model_name}] Epoch {epoch+1}/{max_epochs} 進度: {progress:.1f}% 完成\n")
            if macro_f1 > best_f1:
                best_f1 = macro_f1
                best_state = model.state_dict()
                patience_counter = 0  # macro-F1有上升就歸零
            else:
                patience_counter += 1  # macro-F1沒上升才累加
            if patience_counter >= patience:
                print(f"[{model_name}] Early stopping at epoch {epoch+1}，已完成 {(epoch+1)/max_epochs*100:.1f}% 訓練 (patience={patience}, restore_best_weights=True)")
                break
        # restore_best_weights=True: 訓練結束後自動恢復到macro-F1最好的那次模型參數
        model.load_state_dict(best_state)
        return model

    print("\n--- 訓練最終 ANN 模型 ---")
    final_ann = ANNModel(X.shape[1]).to(device)
    final_ann = train_with_early_stop(final_ann, train_loader_full, val_loader, device, "Final_ANN", max_epochs=10, patience=5)
    torch.save(final_ann.state_dict(), 'final_ann.pth')

    print("\n--- 訓練最終 CNN 模型 ---")
    final_cnn = CNNModel(X.shape[1]).to(device)
    final_cnn = train_with_early_stop(final_cnn, train_loader_full, val_loader, device, "Final_CNN", max_epochs=10, patience=5)
    torch.save(final_cnn.state_dict(), 'final_cnn.pth')

    print("\n--- 訓練最終 RNN 模型 ---")
    final_rnn = RNNModel(X.shape[1]).to(device)
    final_rnn = train_with_early_stop(final_rnn, train_loader_full, val_loader, device, "Final_RNN", max_epochs=10, patience=5)
    torch.save(final_rnn.state_dict(), 'final_rnn.pth')

    print("\n--- 訓練最終 LSTM 模型 ---")
    final_lstm = LSTMModel(X.shape[1]).to(device)
    final_lstm = train_with_early_stop(final_lstm, train_loader_full, val_loader, device, "Final_LSTM", max_epochs=10, patience=5)
    ...
    print("[階段] 儲存模型...")
    # 新增final_meta_model的結果評估
    meta_train_pred = final_meta_model.predict(meta_X_train)
    meta_train_prob = final_meta_model.predict_proba(meta_X_train)[:, 1]
    meta_acc = accuracy_score(y_resampled, meta_train_pred)
    meta_prec = precision_score(y_resampled, meta_train_pred)
    meta_rec = recall_score(y_resampled, meta_train_pred)
    meta_f1 = f1_score(y_resampled, meta_train_pred)
    meta_fpr, meta_tpr, _ = roc_curve(y_resampled, meta_train_prob)
    meta_auc = auc(meta_fpr, meta_tpr)
    print("\nFinal Meta Model 訓練集表現:")
