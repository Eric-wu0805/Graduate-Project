...
def analyze_attack_features():
    ...
    ws_kdd.merge_cells('A1:H1')
    
    # 創建數據列表
    data_rows = []
    data_rows.append(['攻擊類型', '記錄數', '特徵類別', '特徵名稱', '使用率(%)', '最小值', '最大值', '均值', '說明'])
    
    # 分析每個攻擊類型（使用測試集數據進行分析）
    for attack in all_attack_types:
        attack_data = df_test[df_test['labels'] == attack]
        record_count = len(attack_data)
        
        # 如果測試集中沒有該攻擊類型，則使用訓練集數據
        if record_count == 0:
            attack_data = df_train[df_train['labels'] == attack]
            record_count = len(attack_data)
    ...
    # 設置列寬
    ws_kdd.column_dimensions['A'].width = 15
    ws_kdd.column_dimensions['B'].width = 10
    ws_kdd.column_dimensions['C'].width = 20
    ws_kdd.column_dimensions['D'].width = 25
    ws_kdd.column_dimensions['E'].width = 12
    ws_kdd.column_dimensions['F'].width = 12
    ws_kdd.column_dimensions['G'].width = 12
    ws_kdd.column_dimensions['H'].width = 12
    ws_kdd.column_dimensions['I'].width = 30
    ...
    try:
        ...
        # 寫入UNSW-NB15數據
        for row in nb15_data_rows:
            ws_nb15.append(row)
        
        # 設置列寬
        ws_nb15.column_dimensions['A'].width = 15
        ws_nb15.column_dimensions['B'].width = 10
        ws_nb15.column_dimensions['C'].width = 20
        ws_nb15.column_dimensions['D'].width = 25
        ...
        ws_nb15.column_dimensions['I'].width = 30
