...
# Random Masking 工具，支援基於特徵重要性的動態 mask 機率
def _apply_random_masking(X_batch: torch.Tensor, mask_prob: float, mask_value: float = 0.0, feature_weights: dict = None) -> torch.Tensor:
    """
    訓練時的隨機元素遮罩 (element-wise)，根據特徵重要性動態調整 mask 機率。
    
    Args:
        X_batch: 輸入張量，形狀為 (batch_size, features) 或 (batch_size, channels, features)
        mask_prob: 基礎 mask 機率（來自三階段策略）
        mask_value: 被遮罩的填充值
        feature_weights: 特徵重要性字典，鍵為特徵索引，值為重要性分數（0到1）
    ...
    # 創建 mask 機率張量，初始為基礎 mask 機率
    if X_batch.dim() == 3:  # CNN: (batch_size, channels, features)
        mask_probs = torch.ones_like(X_batch[:, 0, :]) * mask_prob
    else:  # ANN/RNN/LSTM: (batch_size, features)
        mask_probs = torch.ones_like(X_batch) * mask_prob
    
    # 根據特徵重要性調整 mask 機率
    if feature_weights:
        for feat_idx, weight in feature_weights.items():
            # 重要性高（weight 接近 1）-> 降低 mask 機率；重要性低（weight 接近 0）-> 提高 mask 機率
    ...
    if X_batch.dim() == 3:
        element_mask = element_mask.unsqueeze(1)  # 適配 CNN 的形狀
    return X_batch.masked_fill(element_mask, mask_value)

# 訓練函數，支援特徵重要性並修改第一階段為固定 mask 機率
def train_model(model, train_loader, val_loader, device='cpu', epochs=10, model_name="", 
    ...
    """
    訓練模型，支持動態類別權重、Early Stopping、三階段 Random Masking 和基於特徵重要性的 mask 調整
    """
    ...
    for epoch in range(epochs):
        ...
        if enable_three_stage_random_masking:
            if epoch < 10:  # 前期 (1-10): 固定 5%
                ...
                stage_name = "前期固定"
            elif epoch < 30:  # 中期 (11-30): 從 5% 增加到 20%
        ...
        for X_batch, y_batch in pbar:
            ...
            if enable_three_stage_random_masking and current_mask_prob > 0:
                ...
                else:  # ANN/RNN/LSTM: (batch, features)
                    X_batch = _apply_random_masking(X_batch, current_mask_prob, mask_value, feature_weights)
...
def main():
    ...
    print(f'ANN result:{ann_pred_train}')
    print(f'CNN result:{cnn_pred_train}')
    print(f'RNN result:{rnn_pred_train}')
    print(f'LSTM result:{lstm_pred_train}')
    ...
    print(f'meta-feature:{meta_X_train}')
    ...
    print("動態類別權重和基於 XGBoost 特徵重要性的 mask 訓練完成（第一階段固定 5%）！")
